class BeerchecksController < ApplicationController
  ...
  def show
    ...
    @weather_viewable = weather_view_generator(Owm_weather.parse_weather(weather))
    @forecast_viewable = forecast_view_generator(Owm_weather.parse_forecast_multiday(forecast))
    render 'show'
  end
